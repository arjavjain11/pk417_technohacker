...
<body>
 ...
 if(isset($_POST['signin'])){
   ...
   $experts_login = "select * from experts where expert_email = '$email' and expert_password = '$password'";
   $user_login_query = mysqli_query($conn,$user_login);
   $experts_login_query = mysqli_query($conn,$experts_login);
   $check_user_query = mysqli_num_rows($user_login_query);
   $check_experts_query = mysqli_num_rows($experts_login_query);
  echo $check_query;
 	if($check_user_query == 1){
       ...
       echo "<script>window.open('home.php?user_name=$user_name','_self')</script>";
        	}else if( $check_experts_query == 1){
        $_SESSION['expert_email'] =$email;
       $user = $_SESSION['expert_email'];
       $get_user = "select * from experts where expert_email='$user'";
       $run_user = mysqli_query($conn,$get_user);
       $row = mysqli_fetch_array($run_user);
       $expert_name = $row['expert_name'];
       echo "<script>window.open('expert_dash.php?expert_name=$expert_name','_self')</script>";	
        	}else {
